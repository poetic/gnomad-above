...
var CampaignSchema = new SimpleSchema({
  ...
  endDate: {
    ...
    custom: function () {
      ...
      if (!endDate) {
        return 'required'
      } else if (endDate < moment().add(-1, 'day').startOf('day').toDate()) {
        return 'currentOrFutureDate'
      }
    },
  ...
  endTime: {
    ...
    decimal: false,
    custom: function () {
      var startDate = this.siblingField('startDate').value.toDateString()
      var endDate   = this.siblingField('endDate').value.toDateString()
      var startTime = this.siblingField('startTime').value
...
CampaignSchema.messages({
  ...
  'currentOrFutureDate':           '[label] cannot be in the past',
